        ...
        <Fragment>
            <div className='border rounded-md w-full p-3 flex gap-2'>
                ...
                <div className='space-y-2'>
                    <h4 className='text-md md:text-lg font-bold'>{pullReq.title}</h4>
                    <p className='text-sm text-gray-600 text-justify'>{pullReq.comment}</p>
                    <p className='text-xs text-gray-600 flex items-center gap-x-1'>
                        ...
                        <span className='text-black font-medium'>#{pullReq._id.slice(20)}</span> by <span className='text-black font-medium'>{pullReq.repository.owner.username}</span> {moment(pullReq.createdAt).startOf('hour').fromNow()}</p>
                </div>
            </div>
        </Fragment>
    )
